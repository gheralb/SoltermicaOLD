model Interacumulador_Orden 
  ...
  Modelica.Thermal.FluidHeatFlow.Sensors.m_flowSensor m_flowSensor(final medium
      =medium) 
  ...
  Soltermica.ClasesBasicas.Control_Acumulador_Orden ControlEstratificacion(
    final medium=medium,
    final n=n,
    final volumen=espec.volumen,
    final altura=espec.altura) annotation (extent=[-20,-40; 0,-20]);
...
equation 
  connect(flowPort_a1, m_flowSensor.flowPort_a) annotation (points=[-100,0; -70,
        0; -70,-10],          style(color=1, rgbcolor={255,0,0}));
  connect(m_flowSensor.flowPort_b, volumenControl.flowPort_a) annotation (
      points=[-70,-30; -70,-40],                   style(color=1, rgbcolor={255,
          0,0}));
  connect(volumenControl.flowPort_b, flowPort_b1) annotation (points=[-70,-60;
        -70,-80; -100,-80], style(color=1, rgbcolor={255,0,0}));
  connect(tSensor.flowPort, flowPort_a1) annotation (points=[-68,14; -86,14;
        -86,0; -100,0], style(
      color=1,
      rgbcolor={255,0,0},
      pattern=0,
  ...
  connect(tSensor.y, ControlEstratificacion.T_in) annotation (points=[-47,14;
        -34,14; -34,-24; -20,-24], style(
      color=74,
      rgbcolor={0,0,127},
      pattern=3,
      gradient=1,
      fillColor=0,
      rgbfillColor={0,0,0}));
  connect(m_flowSensor.y, ControlEstratificacion.m_flow) annotation (points=[-59,-20;
        -40,-20; -40,-30; -20,-30],          style(
      color=74,
      rgbcolor={0,0,127},
      pattern=3,
      gradient=1,
      fillColor=0,
      rgbfillColor={0,0,0}));
  connect(volumenControl.puertoInt, ControlEstratificacion.port_a) annotation (
      points=[-60,-50; -40,-50; -40,-36; -20,-36], style(
      color=42,
      rgbcolor={191,0,0},
      gradient=1,
      fillColor=0,
